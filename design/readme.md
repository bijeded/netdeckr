...
- **Language:** UI copy is **bilingual — Spanish and English** (localized via react-i18next; no hardcoded strings). MTG proper nouns (card names, archetypes: "Izzet Cauldron", "Cori-Steel Cutter") stay in English in *both* locales — it's how the community reads it. This prototype renders the Spanish locale; the English locale mirrors it 1:1 (Fecha→Date, Arquetipo→Archetype, En Tendencia→Trending, Últimos 5 días→Last 5 days).
- **Tone:** terse, data-first, community-native. Labels are short nouns ("Fecha"/"Date", "Arquetipo"/"Archetype", "En Tendencia"/"Trending"). No marketing voice, no full sentences in the UI.
