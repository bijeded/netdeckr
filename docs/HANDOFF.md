...
Backed by a daily MTGTop8 scraper → Supabase (RLS read-only). 131 frontend + 96 scraper tests.
...
1. **Type-based land exclusion for the archetype signature card** (small follow-up to `add-card-art`) — `refresh_archetype_art` excludes only **basic** lands by name (`scraper/supabase_writer.py:_BASIC_LAND_NAMES`), so a 4-of nonbasic land can occasionally win an archetype's "signature card" slot over a signature spell. A type-based signal (store each card's type line, or a maintained nonbasic-land set) would pick a truer signature. Cosmetic; art still renders. (The earlier promo-printing quality issue is **resolved** — the resolver now prefers standard printings over promo/Universes-Beyond variants.)
2. **Re-map existing rows on heuristic changes** — the `--backfill-scryfall` pass keys on `image_url is null`, so once rows are fully enriched a *later* resolver improvement (e.g. the printing-quality or signature-card tweaks) won't refresh them. A `--remap-scryfall` mode that re-resolves all rows (not just imageless ones) would let heuristic changes reach existing data. (Archetype art already recomputes every run, so it's unaffected.)
