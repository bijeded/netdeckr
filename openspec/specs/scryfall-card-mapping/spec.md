...
The system SHALL populate `deck_cards.scryfall_name`, `deck_cards.set_code`, `deck_cards.collector_number`, and `deck_cards.image_url` from the resolved printing when writing deck cards, both for newly scraped decks and, as a one-time backfill, for rows not yet fully enriched. The backfill SHALL use `image_url` (the last column written) as its completeness sentinel, so rows mapped before the image column existed are re-enriched. A resolution miss SHALL leave those columns null; the scraped `card_name` is always retained.
...
- **WHEN** the one-time backfill runs over `deck_cards` rows whose `image_url` is null
