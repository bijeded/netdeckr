...
## Purpose
The persistent app-shell footer: links to the How It Works and Privacy Policy pages, the EN/ES language toggle (relocated here from the filter sidebar so it stays reachable when the sidebar is hidden on legal pages), and the required Wizards of the Coast Fan Content Policy attribution legend. Rendered once, as a sibling of the topbar and main content, visible on both the dashboard and any legal page, and responsive at mobile widths.
