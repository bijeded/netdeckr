...
- [x] 3.1 Render the strip (Events · Archetypes · Decks) right-aligned in the header title row (`marginLeft:auto`), leaving the title, time-frame pill, and freshness unchanged.
- [x] 3.2 Feed the strip from `useMetagame`'s `totals`; when an archetype filter is active, override with the isolated archetype's decks (Archetypes = 1, Decks = its deck count, Events = distinct events among them).
- [x] 3.3 Format numbers with `toLocaleString()` (thousands separators).
- [x] 3.4 App test: strip shows the totals; reflects an event filter; shows 1 archetype under an archetype filter; verify it doesn't disturb the existing header elements.

## 4. Verify

- [x] 4.1 Run `npm run test`, `npm run type-check`, `npm run lint` — all green.
- [x] 4.2 Manual read-only check against live Supabase: strip matches the window (Events/Archetypes/Decks), narrows under event + archetype filters, right-aligned per the design, ES/EN labels correct.
