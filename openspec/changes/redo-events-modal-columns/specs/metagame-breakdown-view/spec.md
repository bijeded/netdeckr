...
#### Scenario: An unrecorded event fact reads as an em dash
- **WHEN** the events modal lists an event with no reported player count, or an event with no recorded date
- **THEN** that row shows an em dash in the corresponding column, keeping its place in the column band, and the row remains selectable and applies the event filter like any other
...
- **THEN** its leading "All events" row spans the full width of the row rather than taking the date, name, and player-count columns, and shows no em dash in place of the facts that do not apply to it
...
- **WHEN** the user opens the Archetypes modal
- **THEN** each row still carries exactly one figure — the archetype's metagame share — with no empty extra figure column introduced
...
- **THEN** all three StatCards are interactive and their modals behave identically, the tier modal's two figure columns remain legible beside their tier labels without the row wrapping onto a second line, and the events modal's date and player-count columns remain legible beside the event name without the row wrapping onto a second line
