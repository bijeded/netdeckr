...
**Direction derived from the window, not hardcoded per label.** The two windows are nested (7 days ⊂ 2 weeks), so the glyph means something: `→` trailing while narrow (activation widens), `←` leading while wide (activation narrows back). Alternative considered: a fixed trailing `→` in both states, which avoids the label's left edge shifting by one glyph when it flips. Chosen against, deliberately — the directional cue is the point; the shift is one glyph on a row that already reflows. **Confirmed on the Vercel preview (PR #200)**: the shift does not read as jumpy next to the title, so the directional glyph stands and the fixed-trailing-`→` fallback is not needed.
...
- **The pill's new interactivity is invisible at rest** → mitigated by cursor + hover, but on touch there is no hover. The glyph is what carries the affordance on phones; **confirmed on the preview (PR #200)** that it reads as interactive at phone width, with the title row still reflowing cleanly alongside the StatCard strip.
