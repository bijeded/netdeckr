...
- [ ] 3.1 Port the design `ChangeIndicator` (arrow **+ number**) as a `ShareDelta` component in `src/components/`, taking `{ direction, value }` + a localized aria-label, rendering nothing when the delta is suppressed (write component tests first).
- [ ] 3.2 Render `ShareDelta` in `ArchetypeCard`'s stat footer, **right-aligned opposite the share %**, distinct from the top-right performance trend arrow; keep both legible over the card surface.
- [ ] 3.3 Test both windows show the share delta (including the 2-week baseline where the performance arrow is absent), and that the two indicators coexist on the 5-day view.

## 4. Localization

- [ ] 4.1 Add `shareDelta.*` aria-label keys (ES/EN) describing the direction + pp change (numeric value + archetype name formatted consistently across locales); keep MTG proper nouns English in both.
- [ ] 4.2 Extend the locale-parity test to cover the new keys.

## 5. Verification

- [ ] 5.1 `npm run lint`, `npm run type-check`, `npm run test` all green.
- [ ] 5.2 Live read-only verification against Supabase across all five formats: a known riser shows ▲ +value, a faller ▼ −value, the 2-week view shows the delta, and a format/archetype with a thin preceding slice suppresses the indicator; tune `DELTA_EPS`/`MIN_PREV_DECKS` if needed.
