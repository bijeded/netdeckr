...
- **Vibe:** dark-mode competitive-gaming telemetry — near-black canvas, electric violet accent, dense and data-first. No emoji except 🏆 (event wins, `WinTrophy`); iconography otherwise `≡ ✕ ⬇ ▲ ▼ – ✓ → ←`, plus inline SVG where a glyph cannot carry the meaning on its own (`src/components/viewIcons.tsx` — the decklist view toggle, whose icon is the entire control on mobile). Prefer a glyph; reach for SVG when two states would otherwise read as the same texture at ~13px.
